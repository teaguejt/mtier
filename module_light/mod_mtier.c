...
/* For this varian of the module: swap slow back to fast. */
int swap_slow_to_fast(struct tier_struct *ts) {
    int rv = 0;
    struct page *source = ts->slow_page;
    struct page *dest = ts->fast_page;
...
int swap_fast_to_slow(struct tier_struct *ts) {
    ...
    if(!ts->fast_page) {
        ...
        printk("mtier swap_fast_to_slow: invalid pte.\n");
        return -EINVAL;
    }
    else if(!ts->ptl) {
        printk("mtier swap_fast_to_slow: invalid ptl.\n");
        return -EINVAL;
    }
...
static int mod_mtier_duplicate(struct task_struct *tsk) {
    ...
    if(mod_iter % ft_bk_iters == 0) {
        ...
        for(i = 0; i < cutoff; i++) {
            ...
            if(!mtier_trylock_page(shuffled_pages[i])) {
                DEBUG_CODE(printk("failed to lock slow page.\n"));
                continue;
            }

            rc = avma_walk_and_copy(shuffled_pages[i], tsk);
            unlock_page(shuffled_pages[i]);
            if(rc) {
                DEBUG_CODE(printk("mtier: avma_walk_and_copy failed. "
                            "error code: %d.\n", rc));
                continue;
            }
