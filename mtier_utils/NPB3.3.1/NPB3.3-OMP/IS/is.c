...
/*  Initialize timers  */
    ...
    /* Initialize the arrays, since using class 'D' causes issues with
     * compiliation. */
    key_array = valloc(sizeof(INT_TYPE) * SIZE_OF_BUFFERS);
    key_buff1 = valloc(sizeof(INT_TYPE) * MAX_KEY);
    key_buff2 = valloc(sizeof(INT_TYPE) * SIZE_OF_BUFFERS);
    partial_verify_vals = valloc(sizeof(INT_TYPE) * TEST_ARRAY_SIZE);

    if(!key_array) {
        printf("Error allocating key array\n");
        return -1;
    }
    if(!key_buff1) {
        printf("Error allocating key buffer 1\n");
        return -1;
    }
    if(!key_buff2) {
        printf("Error allocating key buffer 2\n");
        return -1;
    }
    if(!partial_verify_vals) {
        printf("Error allocating verification values array\n");
        return -1;
    }
...
/*  Generate random number sequence and subsequent keys on all procs */
    ...
    alloc_key_buff();
    mp1 = mprotect((void *)key_array, sizeof(INT_TYPE) * SIZE_OF_BUFFERS,
            PROT_READ);
    if(mp1) {
        printf("Error with key buffer 2 mprotect call.\n");
        return -1;
    }
